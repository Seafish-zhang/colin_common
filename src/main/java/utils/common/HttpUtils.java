...
public class HttpUtils {
	...
	static {
		InputStream is = HttpUtils.class
				.getResourceAsStream("wx_apiclient_cert.p12");
		try {
			KeyStore keystore = KeyStore.getInstance("PKCS12");
			char[] keyPassword = WeiXinConfigUtil.getProperty("wx.mchid")
					.toCharArray(); // 证书密码
	...
	public static String get(String url, Map<String, String> params,
		...
		if (params != null && !params.isEmpty()) {
			...
			for (Entry<String, String> entry : params.entrySet()) {
				...
				try {
					param.append(URLEncoder.encode(entry.getValue(),
							DEFAULT_CHARSET));
	...
	private static SSLConnectionSocketFactory getSSLConnectionSocket() {
		return new SSLConnectionSocketFactory(wx_ssl_context, new String[] {
